...
class CombatReady {
	...
	static doAnimateTurn() {
		...
		for (let e of CombatReady.CHEVRONS)
			e.style.left = "-200px"; 
		...
		AudioHelper.play({src: CombatReady.TURN_SOUND});
	}
	...
	static doAnimateNext() {
		...
		AudioHelper.play({src: CombatReady.NEXT_SOUND});
	...
	static timerTick() {
		...
		if (game.user.isGM) {
			...
			game.socket.emit(CombatReady.SOCKET, {senderId: game.user._id, type: "Number", timetick: CombatReady.TIMECURRENT}, resp =>{
			});
		}
		
		// If we're in the last 25%, tick
		if (CombatReady.TIMECURRENT >= (CombatReady.TIMEMAX/4*3)) {
			AudioHelper.play({src: CombatReady.TICK_SOUND});
		}

		let width = CombatReady.TIMECURRENT/CombatReady.TIMEMAX*100; 
		if (width > 100) {
			CombatReady.timerStop(); 
			AudioHelper.play({src: CombatReady.EXPIRE_SOUND});
		} else
	...
	static timerStart() {
		...
		if (game.user.isGM) {
			// push GM time
			CombatReady.TIMECURRENT = 0; 
			game.socket.emit(CombatReady.SOCKET, {senderId: game.user._id, type: "Number", timetick: CombatReady.TIMECURRENT});
		...
		if (!game.paused) {
			...
			CombatReady.INTERVAL_IDS.push({name: "clock",id: window.setInterval(CombatReady.timerTick, 1000)})
		}
	}
	...
	static timerStop() {
		...
		if (game.user.isGM)
			game.settings.set("combatready","timeractive",false); 
	}
